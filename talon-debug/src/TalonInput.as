...
//			height = Math.ceil(height/lineHeight) * lineHeight;
		...
		private function getStageTextBounds():Rectangle
		{
			...
			var width:Number = node.width.isAuto ? AUTO_CACHE_WIDTH : node.width.toPixels(0, 0, node.ppdp, 0, width, height, 0, 0);
			var height:Number = node.height.isAuto ? AUTO_CACHE_HEIGHT : node.height.toPixels(0, 0, node.ppdp, 0, width, height, 0, 0);
