package talon.browser.desktop.utils
{
	public class DesktopDocumentProperty
	{
		public static const DISPLAY_NAME:String    = "display.name";

		public static const SOURCE_PATH:String     = "source.path";
		public static const SOURCE_IGNORE:String   = "source.ignore";
		public static const EXPORT_PATH:String     = "export.path";
		public static const EXPORT_IGNORE:String   = "export.ignore";
		public static const EXPORT_EXEC:String     = "export.exec";
	}
}
