	...
	public class NativeMenuAdapter
	{
		...
		private function removeNativeMenuItem(item:NativeMenuItem):void
		{
			...
			if (item.menu != null)
			{
				...
				var command:DesignerCommand = DesignerCommand(item.data);
				if (command) command.removeEventListener(Event.CHANGE, onCommandChange);
				delete _itemsByCommand[item];
			}
		}

		public function addItem(path:String, label:String = null, command:DesignerCommand = null, keyEquivalent:String = null):void
		{
			var item:NativeMenuItem = getOrCreateMenuItem(path);
			item.data = command;
		...
		{
			var command:DesignerCommand = DesignerCommand(e.target);
